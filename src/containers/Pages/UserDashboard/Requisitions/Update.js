...
import {initAdminRequisition, editAdminRequisition, getRequisition, putAdminRequisition } from 'store/actions/adminRequisition';
...
const Update = ({
    initAdminRequisition, editAdminRequisition, getRequisition, putAdminRequisition, 
    isPutting, isFetching,
    edited, data, user,
}) => {
    ...
    useEffect(() => {
        ...
        // Get rest of Hiring manager
        const companyData = {};
        
        companyData['company_requisition_id'] = id;
    ...
    }, [getRequisition,initAdminRequisition,id]);
    ...
    const handlePut = (e) => {
        ...
        // Validate all fields
        const errors = Joi.validateToPlainErrors(data, (['recruiter', 'admin'].indexOf(data.user_role) === -1) ? UPDATE_REQUISITION_SCHEMA : UPDATE_REQUISITION_SCHEMA);
        setErrors(errors);
        ...
        putAdminRequisition(id, formData)
            .then(res => {
                notifier.success('Update requisition success!');
                initAdminRequisition();
                history.push('/admin/requisitions/view/'+id); 
            })
            .catch(err => {
                console.log('err',err);
    ...
    useEffect(()=>{
        ...
        user.requisition_permissions.filter(p => p.status == 1 && p.permission == 'U').map(p=>{
                                                ...
                                                >
                                                    {companyHiringManager ?
                                                        ...
                                                        (
                                                            ...
                                                            <Option value={`${manager.id}`}> {manager.hiring_manager_name}</Option>
                                                            ))
...
const mapStateToProps = store => ({
    isFetching: store.rootReducer.adminRequisition.isFetching,
    data: store.rootReducer.adminRequisition.data,
    isPutting: store.rootReducer.adminRequisition.isPutting,
    edited: store.rootReducer.adminRequisition.edited,
